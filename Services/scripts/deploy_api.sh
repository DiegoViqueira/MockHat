...
IMAGE_NAME="905418182087.dkr.ecr.eu-west-1.amazonaws.com/mockhat.com"
...
if [[ -n "$latest_tag" ]]; then
    ...
    docker build -t $IMAGE_NAME:$new_tag -f ./docker/Dockerfile.api .
    aws ecr get-login-password | docker login --username AWS --password-stdin *********.ecr.eu-west-1.amazonaws.com
    docker push $IMAGE_NAME:$new_tag

    aws cloudformation deploy   --template-file ./config/cloudformation-api.yaml   --stack-name mockhat   --parameter-overrides     HostedZoneId="Z07961711AQSYZYJ8EC9C"   VPC=vpc-0a8741ac883394d5b    AppDomainPrefix="api"    PublicSubnetIds="subnet-05a8843fe15747b16,subnet-0c96a572e69454cbd"     ImageUrl="$IMAGE_NAME:$new_tag"   --capabilities CAPABILITY_IAM CAPABILITY_NAMED_IAM
